...
export class SearchPlanets extends Component {
  ...
  getIndex = (value, arr = [this.props.searchPlanet.data.results], prop) => {
  ...
  renderPlanetList() {
    ...
    return this.props.searchPlanet.data.results.map((item, index) => {
       console.log(
         "this.props.searchPlanet.data.results.population",
         this.props.searchPlanet.data.results[0].population
       );
       let arr = this.props.searchPlanet.data.results.population;
     for(var i = 0; i < arr.length; i++) {
        if(arr[i][value] === item) {
           console.log("arr",i) 
          return i;
