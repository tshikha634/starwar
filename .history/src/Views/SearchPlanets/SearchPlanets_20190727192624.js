...
export class SearchPlanets extends Component {
  ...
  componentDidMount() {
   ...
   fields = () => {
    ...
    this.getPlanetList(stringify(fields));
  }
  ...
  async getPlanetList() {
    try {
      ...
      let params = `limit=${this.state.limit}&offset=${this.state.offset}&name=${this.state.name}&id=${this.state.id}`;
  ...
  renderPlanetList() {
    debugger;
    return this.props.searchPlanet.data.results.map((item, index = index + 2) => {
      return (<ul>
            <li key={item.index} value={index} className="list">
              <Row>
                <Col>
                  <Label>Name</Label>
                </Col>
