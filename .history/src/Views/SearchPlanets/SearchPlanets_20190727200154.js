...
export class SearchPlanets extends Component {
  ...
  async getPlanetList() {
    ...
    try {
      ...
      let PlanetList;
      let params = `limit=${this.state.limit}&offset=${this.state.offset}${this.state.name}`;
      if(params){
          PlanetList = await this.SearchService.getPlanetsByName(params);
      }else{
        PlanetList = await this.SearchService.getPlanetsByName();
      }
  ...
  renderPlanetList() {
    ...
    return this.props.searchPlanet.data.results.map((item, index) => {
      return (<ul>
            <li key={item.index} value={index + 3} className="list">
              <Row>
                <Col>
                  <Label>Name</Label>
                </Col>
